...
use nintondo_dogecoin::bip32::{
    ExtendedPrivKey, ExtendedPubKey, Fingerprint, IntoDerivationPath, KeySource,
};
use nintondo_dogecoin::blockdata::opcodes::OP_0;
use nintondo_dogecoin::blockdata::script;
use nintondo_dogecoin::consensus::encode::{deserialize, serialize_hex};
use nintondo_dogecoin::hashes::hex::FromHex;
use nintondo_dogecoin::psbt::{Psbt, PsbtSighashType};
use nintondo_dogecoin::script::PushBytes;
use nintondo_dogecoin::secp256k1::{self, Secp256k1};
...
fn create_transaction() -> Transaction {
    ...
    Transaction {
        ...
        output: vec![
            TxOut {
                value: Amount::from_str_in(output_0.amount, Denomination::Dogecoin)
            ...
            TxOut {
                value: Amount::from_str_in(output_1.amount, Denomination::Dogecoin)
                    .expect("failed to parse amount")
                    .to_sat(),
...
fn signer_one_sign(
    psbt: Psbt,
    key_map: BTreeMap<nintondo_dogecoin::PublicKey, PrivateKey>,
) -> Psbt {
...
fn sign(mut psbt: Psbt, keys: BTreeMap<nintondo_dogecoin::PublicKey, PrivateKey>) -> Psbt {
    let secp = Secp256k1::new();
