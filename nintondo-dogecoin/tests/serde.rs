...
use nintondo_dogecoin::bip32::{ChildNumber, ExtendedPrivKey, ExtendedPubKey, KeySource};
use nintondo_dogecoin::blockdata::locktime::{absolute, relative};
use nintondo_dogecoin::blockdata::witness::Witness;
use nintondo_dogecoin::consensus::encode::deserialize;
use nintondo_dogecoin::hashes::hex::FromHex;
use nintondo_dogecoin::hashes::{hash160, ripemd160, sha256, sha256d, Hash};
use nintondo_dogecoin::psbt::raw::{self, Key, Pair, ProprietaryKey};
use nintondo_dogecoin::psbt::{Input, Output, Psbt, PsbtSighashType};
use nintondo_dogecoin::sighash::{EcdsaSighashType, TapSighashType};
use nintondo_dogecoin::taproot::{self, ControlBlock, LeafVersion, TapTree, TaprootBuilder};
...
fn serde_regression_address() {
    ...
    let addr = Address::p2pkh(&pk, Network::Dogecoin);
