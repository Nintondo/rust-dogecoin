...
// Similar to Taproot Builder in Dogecoin Core.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct TaprootBuilder {
    // The following doc-comment is from Dogecoin Core, but modified for Rust. It describes the
...
mod test {
    ...
    fn control_block_verify() {
        ...
        // test vectors obtained from printing values in feature_taproot.py from Dogecoin Core
    ...
    fn bip_341_tests() {
        ...
        for arr in data["scriptPubKey"].as_array().unwrap() {
            ...
            let addr = Address::p2tr(secp, internal_key, merkle_root, Network::Dogecoin);
