...
//! use nintondo_dogecoin::{Address, PublicKey, Network};
//! use nintondo_dogecoin::secp256k1::{rand, Secp256k1};
//!
...
pub enum Error {
    ...
    /// Dogecoin script opcode does not match any known witness version, the script is malformed.
...
impl From<WitnessVersion> for opcodes::All {
    /// Converts [`WitnessVersion`] instance into corresponding Dogecoin scriptopcode (`OP_0`..`OP_16`).
...
/// use nintondo_dogecoin::address::{NetworkUnchecked, NetworkChecked};
///
/// // variant 1
...
/// ```
/// # use std::str::FromStr;
/// # use nintondo_dogecoin::address::{Address, NetworkChecked};
...
/// ```
/// # use std::str::FromStr;
/// # use nintondo_dogecoin::address::{Address, NetworkUnchecked};
/// let address: Address<NetworkUnchecked> = Address::from_str("132F25rTsvBdp9JzLLBHP5mvGY66i1xdiM")
///                .unwrap();
...
#[cfg(feature = "serde")]
crate::serde_utils::serde_string_serialize_impl!(Address, "a Dogecoin address");
...
impl Address {
    ...
    /// Checks whether or not the address is following Dogecoin standardness rules.
    ///
    /// SegWit addresses with unassigned witness versions or non-standard program sizes are
    /// considered non-standard.
    ...
    /// # let address = ADDRESS.parse::<nintondo_dogecoin::Address<_>>().unwrap().assume_checked();
...
mod tests {
    ...
    #[test]
    fn test_p2wpkh() {
        // stolen from Dogecoin transaction: b3c8c2b6cfc335abbcb2c7823a8453f55d64b2b5125a9a61e8737230cdb8ce20
        let mut key = "033bc8c83c52df5712229a2f72206d90192366c36428cb0c12b6af98324d97bfbc"
            .parse::<PublicKey>()
            .unwrap();
        let addr = Address::p2wpkh(&key, Dogecoin).unwrap();
        ...
        assert_eq!(Address::p2wpkh(&key, Dogecoin), Err(Error::UncompressedPubkey));
    }

    #[test]
    fn test_p2wsh() {
        // stolen from Dogecoin transaction 5df912fda4becb1c29e928bec8d64d93e9ba8efa9b5b405bd683c86fd2c65667
        ...
        let addr = Address::p2wsh(&script, Dogecoin);
        assert_eq!(
            &addr.to_string(),
    ...
    #[test]
    fn test_p2shwpkh() {
        // stolen from Dogecoin transaction: ad3fd9c6b52e752ba21425435ff3dd361d6ac271531fc1d2144843a9f550ad01
        let mut key = "026c468be64d22761c30cd2f12cbc7de255d592d7904b1bab07236897cc4c2e766"
            .parse::<PublicKey>()
            .unwrap();
        let addr = Address::p2shwpkh(&key, Dogecoin).unwrap();
    ...
    #[test]
    fn test_p2shwsh() {
        // stolen from Dogecoin transaction f9ee2be4df05041d0e0a35d7caa3157495ca4f93b233234c9967b6901dacf7a9
        ...
        let addr = Address::p2shwsh(&script, Dogecoin);
    ...
    fn test_non_existent_segwit_version() {
        ...
        let addr = Address::new(Dogecoin, Payload::WitnessProgram(witness_prog));
        roundtrips(&addr);
    }
    ...
    fn test_qr_string() {
        ...
        {
            let addr =
                Address::from_str(el).unwrap().require_network(Network::Dogecoin).expect("mainnet");
    ...
    fn test_valid_networks() {
        ...
        const LEGACY_EQUIVALENCE_CLASSES: &[&[Network]] =
            &[&[Network::Dogecoin], &[Network::Testnet, Network::Regtest, Network::Signet]];
        const SEGWIT_EQUIVALENCE_CLASSES: &[&[Network]] =
            &[&[Network::Dogecoin], &[Network::Regtest], &[Network::Testnet, Network::Signet]];
    ...
    fn p2tr_from_untweaked() {
        ...
        let address = Address::p2tr(&secp, internal_key, None, Network::Dogecoin);
        assert_eq!(
            address.to_string(),
            "bc1p5cyxnuxmeuwuvkwfem96lqzszd02n6xdcjrs20cac6yqjjwudpxqkedrcr"
        );
    ...
    fn test_is_related_to_pubkey_p2tr() {
        ...
        let address = Address::p2tr_tweaked(tweaked_pubkey, Network::Dogecoin);
    ...
    fn test_fail_address_from_script() {
        ...
        assert_eq!(Address::from_script(&bad_p2wpkh, Network::Dogecoin), expected);
        assert_eq!(Address::from_script(&bad_p2wsh, Network::Dogecoin), expected);
        assert_eq!(
            Address::from_script(&invalid_segwitv0_script, Network::Dogecoin),
    ...
    fn test_matches_script_pubkey() {
        ...
        for addr in &addresses {
            let addr = Address::from_str(addr).unwrap().require_network(Network::Dogecoin).unwrap();
            for another in &addresses {
                let another =
                    Address::from_str(another).unwrap().require_network(Network::Dogecoin).unwrap();
