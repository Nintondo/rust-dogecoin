...
impl EcdsaSighashType {
    ...
    pub fn from_consensus(n: u32) -> EcdsaSighashType {
        ...
        // In Dogecoin Core, the SignatureHash function will mask the (int32) value with
...
/// being relayed on today's Dogecoin network.
...
impl<R: BorrowMut<Transaction>> SighashCache<R> {
    ...
    /// ```
    /// use nintondo_dogecoin::{absolute, Transaction, Script};
    /// use nintondo_dogecoin::sighash::{EcdsaSighashType, SighashCache};
    ///
...
mod tests {
    ...
    fn test_sighashes_keyspending() {
        // following test case has been taken from Dogecoin Core test framework
    ...
    fn p2pkh_hex(pk: &str) -> ScriptBuf {
        ...
        Address::p2pkh(&pk, Network::Dogecoin).script_pubkey()
    }
