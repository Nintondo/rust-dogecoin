...
//! Dogecoin network messages.
//!
//! This module defines the `NetworkMessage` and `RawNetworkMessage` types that
//! are used for (de)serializing Dogecoin objects for transmission on the network.
//!
...
/// [Dogecoin Wiki: Protocol Specification](https://en.bitcoin.it/wiki/Protocol_specification)
...
mod test {
    ...
    fn serialize_verack_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Dogecoin), payload: NetworkMessage::Verack }),
    ...
    fn serialize_ping_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Dogecoin), payload: NetworkMessage::Ping(100) }),
    ...
    fn serialize_mempool_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Dogecoin), payload: NetworkMessage::MemPool }),
    ...
    fn serialize_getaddr_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Dogecoin), payload: NetworkMessage::GetAddr }),
    ...
    fn deserialize_getaddr_test() {
        ...
        let preimage = RawNetworkMessage {
            magic: Magic::from(Network::Dogecoin),
    ...
    fn deserialize_version_test() {
        ...
        assert!(msg.is_ok());
        let msg = msg.unwrap();
        assert_eq!(msg.magic, Magic::from(Network::Dogecoin));
