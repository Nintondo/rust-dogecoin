...
//!
//! This module provides various constants relating to the Dogecoin network
...
//!
//! ```rust
//! use nintondo_dogecoin::network::constants::Network;
//! use nintondo_dogecoin::consensus::encode::serialize;
//!
...
impl Network {
    ...
    ///
    /// ```rust
    /// use nintondo_dogecoin::network::constants::{Network, Magic};
    ...
    /// ```rust
    /// use nintondo_dogecoin::network::constants::Network;
    /// use nintondo_dogecoin::blockdata::constants::ChainHash;
    ///
    /// let network = Network::Dogecoin;
    ...
    /// use std::convert::TryFrom;
    ///
    /// assert_eq!(Ok(Network::Dogecoin), Network::try_from(ChainHash::BITCOIN));
    /// ```
...
impl TryFrom<ChainHash> for Network {
    ...
    fn try_from(chain_hash: ChainHash) -> Result<Self, Self::Error> {
        match chain_hash {
            ...
            ChainHash::BITCOIN => Ok(Network::Dogecoin),
...
impl From<Network> for Magic {
    fn from(network: Network) -> Magic {
        match network {
            ...
            Network::Dogecoin => Magic::BITCOIN,
...
impl TryFrom<Magic> for Network {
    ...
    fn try_from(magic: Magic) -> Result<Self, Self::Error> {
        match magic {
            // Note: any new network entries must be matched against here.
            Magic::BITCOIN => Ok(Network::Dogecoin),
...
impl ServiceFlags {
    ...
    /// set by all Dogecoin Core non pruned nodes, and is unset by SPV clients or other light
    /// clients.
    pub const NETWORK: ServiceFlags = ServiceFlags(1 << 0);

    /// GETUTXO means the node is capable of responding to the getutxo protocol request.  Dogecoin
    /// Core does not support this but a patch set called Dogecoin XT does.
    /// See BIP 64 for details on how this is implemented.
    pub const GETUTXO: ServiceFlags = ServiceFlags(1 << 1);

    /// BLOOM means the node is capable and willing to handle bloom-filtered connections.  Dogecoin
...
mod tests {
    ...
    #[test]
    fn serialize_test() {
        assert_eq!(serialize(&Network::Dogecoin.magic()), &[0xf9, 0xbe, 0xb4, 0xd9]);
        ...
        assert_eq!(deserialize(&[0xf9, 0xbe, 0xb4, 0xd9]).ok(), Some(Network::Dogecoin.magic()));
    ...
    #[test]
    fn string_test() {
        assert_eq!(Network::Dogecoin.to_string(), "bitcoin");
        assert_eq!(Network::Testnet.to_string(), "testnet");
        assert_eq!(Network::Regtest.to_string(), "regtest");
        assert_eq!(Network::Signet.to_string(), "signet");

        assert_eq!("bitcoin".parse::<Network>().unwrap(), Network::Dogecoin);
