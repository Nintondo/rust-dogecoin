...
/// The prefix for signed messages using Dogecoin's message signing protocol.
pub const BITCOIN_SIGNED_MSG_PREFIX: &[u8] = b"\x18Dogecoin Signed Message:\n";
...
mod message_signing {
    ...
    /// An error used for dealing with Dogecoin Signed Messages.
    #[cfg_attr(docsrs, doc(cfg(feature = "secp-recovery")))]
    ...
    /// A signature on a Dogecoin Signed Message.
    ///
...
/// Hash message for signature using Dogecoin's message signing format.
...
mod tests {
    ...
    fn test_message_signature() {
        ...
        let p2pkh = Address::p2pkh(&pubkey, Network::Dogecoin);
        assert_eq!(signature2.is_signed_by_address(&secp, &p2pkh, msg_hash), Ok(true));
        let p2wpkh = Address::p2wpkh(&pubkey, Network::Dogecoin).unwrap();
        assert_eq!(
            signature2.is_signed_by_address(&secp, &p2wpkh, msg_hash),
            Err(MessageSignatureError::UnsupportedAddressType(AddressType::P2wpkh))
        );
        let p2shwpkh = Address::p2shwpkh(&pubkey, Network::Dogecoin).unwrap();
        assert_eq!(
