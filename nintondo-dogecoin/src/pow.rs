...
impl Work {
    ...
    /// used mainly for informative and displaying purposes, similarly to Dogecoin Core's
...
impl Target {
    /// When parsing nBits, Dogecoin Core converts a negative target threshold into a target of zero.
    ...
    ///
    /// ref: <https://en.bitcoin.it/wiki/Target>
    // In Dogecoind this is ~(u256)0 >> 32 stored as a floating-point type so it gets truncated, hence
    ...
    /// defined for the Dogecoin network and `current` is the current [target] for this block. As
...
/// in the original version of Dogecoin, likely copying an idea from OpenSSL.
///
...
mod tests {
    ...
    #[test]
    fn work_log2() {
        // Compare work log2 to historical Dogecoin Core values found in Core logs.
