...
impl Denomination {
    ...
    fn precision(self) -> i8 {
        match self {
            Denomination::Dogecoin => -8,
            Denomination::CentiDogecoin => -6,
            Denomination::MilliDogecoin => -5,
            Denomination::MicroDogecoin => -2,
            Denomination::NanoDogecoin => 1,
            Denomination::PicoDogecoin => 4,
            Denomination::Bit => -2,
    ...
    fn as_str(self) -> &'static str {
        match self {
            Denomination::Dogecoin => "BTC",
            Denomination::CentiDogecoin => "cBTC",
            Denomination::MilliDogecoin => "mBTC",
            Denomination::MicroDogecoin => "uBTC",
            Denomination::NanoDogecoin => "nBTC",
            Denomination::PicoDogecoin => "pBTC",
...
impl FromStr for Denomination {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match denomination_from_str(s) {
            ...
            Some(D::MilliDogecoin) | Some(D::PicoDogecoin) | Some(D::MilliSatoshi)
                if starts_with_uppercase() =>
            {
                Err(PossiblyConfusingDenomination(s.to_owned()))
            }
            Some(D::NanoDogecoin) | Some(D::MicroDogecoin) if starts_with_uppercase() => {
...
fn denomination_from_str(mut s: &str) -> Option<Denomination> {
    if s.eq_ignore_ascii_case("BTC") {
        return Some(Denomination::Dogecoin);
    }

    if s.eq_ignore_ascii_case("cBTC") {
        return Some(Denomination::CentiDogecoin);
    }

    if s.eq_ignore_ascii_case("mBTC") {
        return Some(Denomination::MilliDogecoin);
    }

    if s.eq_ignore_ascii_case("uBTC") {
        return Some(Denomination::MicroDogecoin);
    }

    if s.eq_ignore_ascii_case("nBTC") {
        return Some(Denomination::NanoDogecoin);
    }

    if s.eq_ignore_ascii_case("pBTC") {
        return Some(Denomination::PicoDogecoin);
    }
...
/// Amount
///
/// The [Amount] type can be used to express Dogecoin amounts that support
...
impl Amount {
    ...
    pub fn from_btc(btc: f64) -> Result<Amount, ParseAmountError> {
        Amount::from_float_in(btc, Denomination::Dogecoin)
    }
    ...
    /// Express this [`Amount`] as a floating-point value in Dogecoin.
    ///
    /// Please be aware of the risk of using floating-point numbers.
    ///
    /// # Examples
    /// ```
    /// # use nintondo_dogecoin::{Amount, Denomination};
    /// let amount = Amount::from_sat(100_000);
    /// assert_eq!(amount.to_btc(), amount.to_float_in(Denomination::Dogecoin))
    /// ```
    pub fn to_btc(self) -> f64 {
        self.to_float_in(Denomination::Dogecoin)
    }
...
// Just using Dogecoin denominated string.
impl fmt::Display for Amount {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.fmt_value_in(f, Denomination::Dogecoin)?;
        write!(f, " {}", Denomination::Dogecoin)
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match &self.style {
            ...
            DisplayStyle::DynamicDenomination if self.sats_abs >= Amount::ONE_BTC.to_sat() => {
                fmt_satoshi_in(self.sats_abs, self.is_negative, f, Denomination::Dogecoin, true, format_options)
            },
            DisplayStyle::DynamicDenomination => {
...
/// SignedAmount
///
/// The [SignedAmount] type can be used to express Dogecoin amounts that support
/// arithmetic and conversion to various denominations.
///
///
/// Warning!
...
impl SignedAmount {
    ...
    pub fn from_btc(btc: f64) -> Result<SignedAmount, ParseAmountError> {
        SignedAmount::from_float_in(btc, Denomination::Dogecoin)
    }
    ...
    /// Express this [`SignedAmount`] as a floating-point value in Dogecoin.
    ///
    /// Equivalent to `to_float_in(Denomination::Dogecoin)`.
    ///
    /// Please be aware of the risk of using floating-point numbers.
    pub fn to_btc(self) -> f64 {
        self.to_float_in(Denomination::Dogecoin)
...
pub mod serde {
    ...
    //!     #[serde(with = "nintondo_dogecoin::amount::serde::as_btc")]
    //!     pub amount: Amount,
    //! }
    //! ```
    ...
    impl SerdeAmount for Amount {
        ...
        fn ser_btc<S: Serializer>(self, s: S) -> Result<S::Ok, S::Error> {
            f64::serialize(&self.to_float_in(Denomination::Dogecoin), s)
        }
        fn des_btc<'d, D: Deserializer<'d>>(d: D) -> Result<Self, D::Error> {
            use serde::de::Error;
...
mod tests {
    ...
    fn floating_point() {
        ...
        assert_eq!(f(11.22, D::Dogecoin), Ok(sat(1122000000)));
        assert_eq!(sf(-11.22, D::MilliDogecoin), Ok(ssat(-1122000)));
        assert_eq!(f(11.22, D::Bit), Ok(sat(1122)));
        assert_eq!(sf(-1000.0, D::MilliSatoshi), Ok(ssat(-1)));
        assert_eq!(f(0.0001234, D::Dogecoin), Ok(sat(12340)));
        assert_eq!(sf(-0.00012345, D::Dogecoin), Ok(ssat(-12345)));
        ...
        assert_eq!(sf(-100.0, D::MilliSatoshi), Err(ParseAmountError::TooPrecise));
        assert_eq!(f(42.123456781, D::Dogecoin), Err(ParseAmountError::TooPrecise));
        assert_eq!(sf(-184467440738.0, D::Dogecoin), Err(ParseAmountError::TooBig));
        ...
        assert_eq!(btc(2.5).to_float_in(D::Dogecoin), 2.5);
        assert_eq!(btc(-2.5).to_float_in(D::MilliDogecoin), -2500.0);
        ...
        assert_eq!(&btc(0.0012).to_float_in(D::Dogecoin).to_string(), "0.0012")
    }
    ...
    fn parsing() {
        ...
        // exactly 50 chars.
        assert_eq!(
            p("100000000000000.0000000000000000000000000000000000", Denomination::Dogecoin),
            Err(E::TooBig)
        );
        // more than 50 chars.
        assert_eq!(
            p("100000000000000.00000000000000000000000000000000000", Denomination::Dogecoin),
            Err(E::InputTooLarge)
        );
    }
    ...
    fn to_string() {
        ...
        assert_eq!(Amount::ONE_BTC.to_string_in(D::Dogecoin), "1");
        assert_eq!(format!("{:.8}", Amount::ONE_BTC.display_in(D::Dogecoin)), "1.00000000");
        assert_eq!(Amount::ONE_BTC.to_string_in(D::Satoshi), "100000000");
        assert_eq!(Amount::ONE_SAT.to_string_in(D::Dogecoin), "0.00000001");
        assert_eq!(SignedAmount::from_sat(-42).to_string_in(D::Dogecoin), "-0.00000042");

        assert_eq!(Amount::ONE_BTC.to_string_with_denomination(D::Dogecoin), "1 BTC");
        ...
        assert_eq!(Amount::ONE_SAT.to_string_with_denomination(D::Dogecoin), "0.00000001 BTC");
        assert_eq!(
            SignedAmount::from_sat(-42).to_string_with_denomination(D::Dogecoin),
            "-0.00000042 BTC"
        );
    }
    ...
    fn to_from_string_in() {
        ...
        assert_eq!("0.00253583", Amount::from_sat(253583).to_string_in(D::Dogecoin));
        assert_eq!("-5", SignedAmount::from_sat(-5).to_string_in(D::Satoshi));
        assert_eq!("0.1", Amount::from_sat(100_000_00).to_string_in(D::Dogecoin));
        assert_eq!("-100", SignedAmount::from_sat(-10_000).to_string_in(D::Bit));
        assert_eq!("2535830", Amount::from_sat(253583).to_string_in(D::NanoDogecoin));
        assert_eq!("-100000", SignedAmount::from_sat(-10_000).to_string_in(D::NanoDogecoin));
        assert_eq!("2535830000", Amount::from_sat(253583).to_string_in(D::PicoDogecoin));
        assert_eq!("-100000000", SignedAmount::from_sat(-10_000).to_string_in(D::PicoDogecoin));
        ...
        assert_eq!(
            "0.10000000",
            format!("{:.8}", Amount::from_sat(100_000_00).display_in(D::Dogecoin))
        );
        ...
        assert_eq!(ua_str(&ua_sat(500).to_string_in(D::Dogecoin), D::Dogecoin), Ok(ua_sat(500)));
        assert_eq!(
            ua_str(&ua_sat(21_000_000).to_string_in(D::Bit), D::Bit),
            Ok(ua_sat(21_000_000))
        );
        assert_eq!(
            ua_str(&ua_sat(1).to_string_in(D::MicroDogecoin), D::MicroDogecoin),
            Ok(ua_sat(1))
        );
        assert_eq!(
            ua_str(&ua_sat(1_000_000_000_000).to_string_in(D::MilliDogecoin), D::MilliDogecoin),
            Ok(ua_sat(1_000_000_000_000))
        );
        assert_eq!(
            ua_str(&ua_sat(u64::max_value()).to_string_in(D::MilliDogecoin), D::MilliDogecoin),
            Err(ParseAmountError::TooBig)
        );

        assert_eq!(
            sa_str(&sa_sat(-1).to_string_in(D::MicroDogecoin), D::MicroDogecoin),
            Ok(sa_sat(-1))
        );

        assert_eq!(
            sa_str(&sa_sat(i64::max_value()).to_string_in(D::Satoshi), D::MicroDogecoin),
            Err(ParseAmountError::TooBig)
        );
        // Test an overflow bug in `abs()`
        assert_eq!(
            sa_str(&sa_sat(i64::min_value()).to_string_in(D::Satoshi), D::MicroDogecoin),
            Err(ParseAmountError::TooBig)
        );

        assert_eq!(
            sa_str(&sa_sat(-1).to_string_in(D::NanoDogecoin), D::NanoDogecoin),
            Ok(sa_sat(-1))
        );
        assert_eq!(
            sa_str(&sa_sat(i64::max_value()).to_string_in(D::Satoshi), D::NanoDogecoin),
            Err(ParseAmountError::TooPrecise)
        );
        assert_eq!(
            sa_str(&sa_sat(i64::min_value()).to_string_in(D::Satoshi), D::NanoDogecoin),
            Err(ParseAmountError::TooPrecise)
        );

        assert_eq!(
            sa_str(&sa_sat(-1).to_string_in(D::PicoDogecoin), D::PicoDogecoin),
            Ok(sa_sat(-1))
        );
        assert_eq!(
            sa_str(&sa_sat(i64::max_value()).to_string_in(D::Satoshi), D::PicoDogecoin),
            Err(ParseAmountError::TooPrecise)
        );
        assert_eq!(
            sa_str(&sa_sat(i64::min_value()).to_string_in(D::Satoshi), D::PicoDogecoin),
            Err(ParseAmountError::TooPrecise)
        );
    }
    ...
    fn to_string_with_denomination_from_str_roundtrip() {
        ...
        assert_eq!(Amount::from_str(&denom(amt, D::Dogecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::MilliDogecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::MicroDogecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Bit)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Satoshi)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::NanoDogecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::MilliSatoshi)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::PicoDogecoin)), Ok(amt));
