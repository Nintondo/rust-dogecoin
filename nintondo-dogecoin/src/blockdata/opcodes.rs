...
//!
//! Dogecoin's script uses a stack-based assembly language. This module defines
...
///
///   Dogecoin Core's `IsPushOnly` considers `OP_RESERVED` to be a "push code", allowing this opcode
...
macro_rules! all_opcodes {
    ($($op:ident => $val:expr, $doc:expr);*) => {
        ...
        ///
        /// The `all` module is provided so one can use a wildcard import `use nintondo_dogecoin::opcodes::all::*` to
