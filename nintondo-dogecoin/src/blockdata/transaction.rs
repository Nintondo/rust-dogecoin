...
impl OutPoint {
    ...
    ///
    /// ```rust
    /// use nintondo_dogecoin::constants::genesis_block;
    /// use nintondo_dogecoin::network::constants::Network;
    ///
    /// let block = genesis_block(Network::Dogecoin);
...
/// Dogecoin transaction input sequence number.
///
...
impl Sequence {
    ...
    /// is equivalent and improves readability for those not steeped in Dogecoin folklore.
    ///
    /// ## Historical note
    ///
    /// The term 'final' is an archaic Dogecoin term, it may have come about because the sequence
    /// number in the original Dogecoin code was intended to be incremented in order to replace a
    /// transaction, so once the sequence number got to `u64::MAX` it could no longer be increased,
    /// hence it was 'final'.
    ///
    ///
    /// Some other references to the term:
    /// - `CTxIn::SEQUENCE_FINAL` in the Dogecoin Core code.
    ...
    /// to 512 seconds.
    ///
    /// Encoding finer granularity of time for relative lock-times is not supported in Dogecoin
    #[inline]
...
impl<E> EncodeSigningDataResult<E> {
    ...
    ///
    /// ```rust
    /// # use nintondo_dogecoin::consensus::deserialize;
    /// # use nintondo_dogecoin::sighash::{LegacySighash, SighashCache};
    /// # use nintondo_dogecoin::Transaction;
    /// # use bitcoin_hashes::{Hash, hex::FromHex};
    /// # let mut writer = LegacySighash::engine();
    /// # let input_index = 0;
    /// # let script_pubkey = nintondo_dogecoin::ScriptBuf::new();
...
///
/// See [Dogecoin Wiki: Transaction][wiki-transaction] for more information.
///
...
impl Transaction {
    ...
    /// is different to what is implemented in Dogecoin Core. The computation should be the same for
...
mod tests {
    ...
    fn test_is_coinbase() {
        ...
        let genesis = constants::genesis_block(Network::Dogecoin);
    ...
    fn test_transaction_verify() {
        ...
        match spending
            ...
            .unwrap()
        {
            script::Error::DogecoinConsensus(_) => {}
