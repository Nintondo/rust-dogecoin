...
//!
//! <details>
//! <summary>What is Dogecoin script</summary>
//!
//! Scripts define Dogecoin's digital signature scheme: a signature is formed
...
//! the script leaves `TRUE` on the stack after being evaluated. Dogecoin's
...
//!
//! See [Dogecoin Wiki: Script][wiki-script] for more information.
//!
...
//! and forcing them to be larger would waste memory and, in case of Dogecoin script, even some
...
/// more than 4 bytes, this is in line with Dogecoin Core (see [`CScriptNum::serialize`]).
///
...
///
/// This code is based on the `CScriptNum` constructor in Dogecoin Core (see `script.h`).
pub fn read_scriptint(v: &[u8]) -> Result<i64, Error> {
    ...
    // Comment and code copied from Dogecoin Core:
...
pub enum Error {
    ...
    DogecoinConsensus(bitcoinconsensus::Error),
...
impl std::error::Error for Error {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        ...
        match *self {
            ...
            | Serialization => None,
            #[cfg(feature = "bitcoinconsensus")]
            DogecoinConsensus(ref e) => Some(bitcoinconsensus_hack::wrap_error(e)),
        }
    }
}
